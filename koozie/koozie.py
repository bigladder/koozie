""" The koozie module supplies conversion wrapper functions for the pint module."""
...
def sorting_function(x):  # pylint: disable=C0103
    """For OrderedDict of units"""
    return len(x[0])


def get_unit_list():
    """Return an OrderedDict of units sorted by length"""
    unit_list = {}
    for u in ureg:  # pylint: disable=C0103
        if u in ureg:  # Certain symbols do not show up
            ...
            unit = list(ureg[u]._units._d.keys())[0]  # pylint: disable=W0212
    ...
    for dim in ureg._dimensions:  # pylint: disable=W0212
        if dim in unit_list:
            continue
        dimensionality = f"{ureg.get_dimensionality(ureg._dimensions[dim].reference)}"  # pylint: disable=W0212
